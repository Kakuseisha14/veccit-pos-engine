...
- El servicio expone el puerto **`5433`** del runner (`5433:5432`) y monta `docker/postgres/init.sql`, que crea automáticamente la base **`veccit_pos_test`** (`CREATE DATABASE veccit_pos_test`). El job env apunta a `DB_HOST=localhost`, `DB_PORT=5433`, `DB_DATABASE=veccit_pos_test`, `NODE_ENV=test` + credenciales e2e (`JWT_SECRET`, `SUPER_ADMIN_EMAIL`, `SUPER_ADMIN_PASSWORD`, `UPLOADS_DIR`).
- **Env FORZADO en las suites (regla crítica):** las suites usan `process.env.X = valor` en `beforeAll` para fijar SIEMPRE `DB_PORT=5433`, `DB_DATABASE=veccit_pos_test` y las credenciales e2e (`SUPER_ADMIN_EMAIL=super@test.com`, `SUPER_ADMIN_PASSWORD=superadmin123`). NO usar `??=` ni `||=`: al importar `AppModule`, `ConfigModule.forRoot()` ya cargó el `.env` del repo en `process.env` ANTES de ejecutarse el `beforeAll`, así que `??=` respeta esos valores y el bootstrap crea el super admin con el email del `.env` (login 401 en los tests). Por eso la estrategia es la inversa: el **CI se alinea a los valores que las suites fuerzan** (`5433:5432`, `init.sql` crea `veccit_pos_test`, job env idéntico), nunca al revés.
...
- El `init.sql` del contenedor se aplica solo en el primer arranque del volumen (base local `veccit_pos`); en CI el volumen es efímero y siempre se crea la base de test.
- **Despliegue:** montar `docker/postgres/init.sql` en el servicio de GH Actions vía `volumes:` es lo que garantiza que `veccit_pos_test` exista en CI; sin ese mount el job conecta a una DB inexistente y entra en bucle de reintentos.
